...
void VidStream<2>::capture_loop() {
	...
	while (running) {
		...
		cv::addWeighted(tmp.frame[0], frame_weight, avg.frame[0], avg_weight, 0,
				avg.frame[0]);
		cv::addWeighted(tmp.frame[1], frame_weight, avg.frame[1], avg_weight, 0,
				avg.frame[1]);
#endif
...
#endif
		resize(frame.frame[i], frame_out.frame[i], cv::Size(), scaling_factor,
