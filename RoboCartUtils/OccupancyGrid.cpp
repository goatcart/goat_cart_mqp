...
void OccupancyGrid::compute(cv::Mat &disparity, cv::Mat &dispOccupancy, cv::Mat &image3d) {
	...
	for (int i = 0; i < image3d.rows; i++) {
		for (int j = 0; j < image3d.cols; j++) {
			...
			if(pt.x > xRange.end || pt.x < xRange.start ||
					h > yRange.end || h < yRange.start ||
					pt.z > zRange.end || pt.z < zRange.start) {
				continue;
			}
			...
			int row = occupancySize.height - floor(scaledZ*occupancySize.height);
	...
	for (int i = 0; i < occupancy.rows; i++) {
		for (int j = 0; j < occupancy.cols; j++) {
			if(occupancy.at<float>(i,j) > 0) {
				float xPt = j, yPt = i;
				float distanceToCamera = sqrt((xCam-xPt)*(xCam-xPt)+(yCam-yPt)*(yCam-yPt));
				...
				float avgHeight = occupancy.at<float>(i,j) > 0 ? height.at<float>(i,j)/occupancy.at<float>(i,j) : 0;
				float pijHeight = 1-exp(-avgHeight/deltaH);
				float lijHeight = pijHeight != 1 ? log(pijHeight/(1-pijHeight)) : 0;
	...
	cv::Mat mask;
	cv::inRange(dispOccupancy, cv::Scalar(255, 0, 0), cv::Scalar(255, 0, 0), mask);
